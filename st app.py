    ...
    with st.container():
        ...
        if uploaded_file is not None:
            ...
            if model:
                ...
                st.markdown(f"<div class='card'><pre style='color:#000000;background:{CLR_CARD};border-radius:10px;padding:10px;margin-bottom:0;'>{details}</pre></div>", unsafe_allow_html=True)
                if pred > 0.5:
